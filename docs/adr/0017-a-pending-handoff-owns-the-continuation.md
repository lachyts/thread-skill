...
other than the closer or its subagents has a cwd in its `<home>` or in the directory its
`Run from:` line names (a `_shared` doc's consumer runs from that seat, never from `_shared`);
a clean or unavailable `ListAgents`/`list_agents` is no evidence (estate METHOD K41
correction), a positive one keeps the doc, and Claude Code's `ListAgents` rows carry no cwd as
of 2026-09-25, so on Claude the 24 h floor is the whole guard and close skips the call; and
a doc already gone is reported, never `git rm`'d — `handoff-lifecycle.md` § Close-out. The
body's claim that the consumed rule is decidable after a compaction "with no half of it left
