...
**The lookup** below is `${CLAUDE_PLUGIN_ROOT}/skills/open/SKILL.md` § Repo-thread lookup (its contract: tiers, exclusions, the CWD tiebreak), run from this file as `slug=<slug> bash "${CLAUDE_PLUGIN_ROOT}/skills/open/scripts/repo-thread-lookup.sh"` in one Bash call, never by copying open's snippet: read as a file, open's text keeps `${CLAUDE_PLUGIN_ROOT}` literal, and the Bash tool leaves it unset, so the copied snippet exits 2. A non-zero exit is a **failed lookup**, never "no hit": stop resolving (rungs 3–5 are not tried), skip the thread update, and report `Needs your call: thread lookup failed (<first stderr line>)` in step 8.
