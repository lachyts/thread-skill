...
```
/wave:plan GifLab                       # default: all open tasks linked to [[GifLab]]
/wave:plan [[GifLab]]                   # explicit wikilink form
/wave:plan GifLab --regenerate          # re-plan even tasks that already have wave: set
/wave:plan --tasks task-a,task-b,task-c # explicit set instead of project filter
```
...
1. `/wave:plan GifLab` from any Claude Code session
...
The GifLab backlog is a good fixture because it exercises both a merge and a deliberate non-merge in one run. Against the open backlog, `/wave:plan GifLab --regenerate` should:
